...
\beta \sim {\rm Normal}(\mu=0, \sigma=1)
$$

ベータ分布の $\alpha$ は過去のホームラン回数、 $\beta$ は過去データの全試行回数を入力します。
正規分布に従う $\beta$ は後に導入されるロジスティック回帰の重みの事前分布を指します( $\beta$ がかぶってしまった)。
...
$$
p = \sigma({\rm logit}(p_{\rm season}) + \beta_0 + \beta_{\rm 1b} \cdot {\rm 1塁の走者の有無} + \beta_{\rm 2b} \cdot {\rm 2塁の走者の有無} + \cdots + \beta_{\rm out} \cdot {\rm アウトカウント})
$$
